...
@interface SDResourceSheetView ()<UITableViewDelegate,
...
@implementation SDResourceSheetView
...
- (void)makeSubviewsLayout {
    ...
    self.frame = CGRectMake(0, SD_ScreenHeight - height, SD_ScreenWidth, height);
    self.tableView.frame = CGRectMake(0, 0, SD_ScreenWidth, height);
}
...
- (void)tapHandle:(UIGestureRecognizer *)tap {
    if ([self.delegate respondsToSelector:@selector(SDResourceSheetView:seletedMode:)]) {
        [self.delegate SDResourceSheetView:self seletedMode:ResourceModeNone];
    }
    [self hide];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(cell == nil){
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SD_ScreenWidth, kDefaultCellHeight)];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    UIView *header = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SD_ScreenWidth, height)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(SDResourceSheetView:seletedMode:)]) {
        [self.delegate SDResourceSheetView:self seletedMode:mode];
    }    
    [self hide];
}
